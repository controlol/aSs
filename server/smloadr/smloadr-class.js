...
if (DEBUG) console.warn("!!! debugging is enabled, to turn off change config file !!!")
...
let smloadrClass = class {
    constructor() {
        this.tokens = new Array(); // array containing the arl, axios config, and unofficialApiQueries
    ...
    setDownloadPath(downloadLocation) {
        return (downloadLocation.charAt(downloadLocation.length-1) !== "/") ? downloadLocation += '/' : downloadLocation;
    }
    ...
    getAxiosErrorConfigSlim(config) {
        return {
            url: config.url,
    ...
    createNewToken(arl) {
        return new Promise((resolve, reject) => {
            if (this.getTokenNR(arl) !== -1) return resolve("arl already exists")
    ...
    initAPI(token_nr) {
        return new Promise((resolve, reject) => {
            return axios.get(unofficialApiUrl+querystring.stringify(Object.assign({}, this.tokens[token_nr].unofficialApiQueries, {method: 'deezer.getUserData', cid: this.getApiCid()})), this.tokens[token_nr].config)
            .then(response => {
                if (!response || Object.keys(response.data.error).length > 0) {
                    ...
                    if (response.data.results && response.data.results.checkForm) {
                        ...
                        return resolve("connected to API")
                    } else {
                        return reject("no checkForm");
                    }
                } else {
                    return reject("wrong deezer credentials")
                }
            })
            .catch(err => {
                if (err.isAxiosError) return reject(new XHRerror("Could not initialize Deezer API", this.getAxiosErrorConfigSlim(err.config)))
                return reject(err)
            })
        })
    }
    ...
    startDownload(track_id = null, arl = null) {
        return new Promise(async (resolve, reject) => {
            try {
                if (!track_id || !arl) throw new downloadError("missing paramater")

                if (this.getTokenNR(arl) === -1) await this.createNewToken(arl)
                ...
                console.log(track_id, "download started")
                ...
                .catch(err => {
                    ...
                    console.error(track_id, "download failed")
                    console.timeEnd(track_id)
                    reject(err)
                })
            } catch (err) {
    ...
    processDownload(track_id, trackInfos = {}, albumInfos = {}, isAlternativeTrack = false) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                let artistName = trackInfos.ALB_ART_NAME.sanitizeFile();
                if (artistName.trim() === '') artistName = 'Unknown artist';

                let albumName = trackInfos.ALB_TITLE.sanitizeFile();
                if (albumName.trim() === '') albumName = 'Unknown album'; 
                
                if (trackQuality.id === musicQualities.FLAC.id) fileExtension = 'flac';

                if (optimizedFS) {
                    saveFilePath = path.join(DOWNLOAD_DIR, artistName, albumName);
                    let artistPath = path.dirname(saveFilePath)
                ...
                saveFilePath += artistName + ' - ' + trackInfos.SNG_TITLE_VERSION.sanitizeFile() + '.' + fileExtension;
                ...
                let downloadMessageAppend = 'download succesful';
                ...
                const albumArtBuffer = await this.downloadAlbumCover(trackInfos, token_nr)
                ...
                await this.addTagsAndStore(decryptedTrackBuffer, trackInfos, saveFilePath, albumArtBuffer)

                if (saveAlbumArt) this.saveAlbumArtBuffer(albumArtBuffer, saveFilePath);
                if (saveLyrics && trackInfos.LYRICS && trackInfos.LYRICS.LYRICS_SYNC_JSON) this.saveSyncedLyrics(trackInfos.LYRICS.LYRICS_SYNC_JSON, saveFilePath)
        ...
        return new Promise((resolve, reject) => {
            ...
            .then(async response => {
                if (response && Object.keys(response.data.error).length === 0 && response.data.results && response.data.results.DATA) {
                    ...
                    return reject(new XHRerror("could not get TrackInfo", response.status, this.getAxiosErrorConfigSlim(response.config)))
                }
            })
            .catch(err => {
                if (err.isAxiosError) return reject(new XHRerror("could not get TrackInfo", this.getAxiosErrorConfigSlim(err.config)))
                return reject(err)
            })
        })
    }
        ...
        return new Promise(resolve => {
            ...
            .then(async response => {
                if (response && Object.keys(response.data.error).length === 0 && response.data.results && response.data.results.DATA && response.data.results.SONGS) {
                    ...
                    return resolve(albumInfos);
                } else if (response.data.error && response.data.error.VALID_TOKEN_REQUIRED) {
                    await this.initAPI(token_nr)

                    return this.getAlbumInfos(id, token_nr)
                    .then(albumInfos => { return resolve(albumInfos) })
                    .catch(() => { return resolve({}) })
                } else {
                    if (DEBUG )console.warn(new XHRerror(JSON.stringify(response.data.error, null, 2), response.status, this.getAxiosErrorConfigSlim(response.config)))
                    return resolve({})
                }
            })
            .catch(err => {
                if (err.isAxiosError && DEBUG) {
                    console.warn(new XHRerror("no response from unofficial deezer API", err.response.status, this.getAxiosErrorConfigSlim(err.config)))
                } else if (DEBUG) {
                    console.warn(err)
                }
                return resolve({}) 
        ...
        return new Promise(resolve => {
            ...
            .then(async response => {
                if (response && 0 === Object.keys(response.data.error).length && response.data.results && response.data.results.LYRICS_ID) {
                    ...
                    if (DEBUG) console.warn(new XHRerror(response.data.error, response.status, this.getAxiosErrorConfigSlim(err.config)))
                    return resolve(null);
                }
     ...
     * @param {Number} numberRetry
     */
    downloadAlbumCover(trackInfos, token_nr, numberRetry = 0) {
        return new Promise(resolve => {
                ...
                .catch(err => {
                    if (err.response.status === 403) {
                        if (numberRetry > 3) {
                            ...
                            setTimeout(() => {
                                return this.downloadAlbumCover(trackInfos, token_nr, numberRetry)
                                .then(albumArtBuffer => { return resolve(albumArtBuffer) })
                                .catch(() => { return resolve(null) })
                            }, 500);
                        } else {
                            return resolve(null);
                        ...
                        if (DEBUG) console.warn(new XHRerror("no response from unofficial deezer API", err.response.status, this.getAxiosErrorConfigSlim(response.config)))
                        return resolve()
                    } else {
    ...
    addTagsAndStore(decryptedTrackBuffer, trackInfos, saveFilePath, albumArtBuffer = null, numberRetry = 0) {
        return new Promise((resolve, reject) => {
            try {
                let trackMetadata = {
                    title: '',
                ...
                let saveFilePathExtension = path.extname(saveFilePath);
                ...
                if ('.mp3' === saveFilePathExtension) {
                    ...
                    processor.on('preprocess', (mdb) => {
                        ...
                        if (flacMetadata.Processor.MDB_TYPE_VORBIS_COMMENT === mdb.type) {
                            mdb.remove();
                        } else if (albumArtBuffer && flacMetadata.Processor.MDB_TYPE_PICTURE === mdb.type) {
                            mdb.remove();
                        }

                        if (mdb.isLast) {
                            mdbVorbisComment = flacMetadata.data.MetaDataBlockVorbisComment.create(!albumArtBuffer, vendor, flacComments);

                            if (albumArtBuffer) mdbVorbisPicture = flacMetadata.data.MetaDataBlockPicture.create(true, 3, 'image/jpeg', '', 1400, 1400, 24, 0, albumArtBuffer);
    ...
    saveAlbumArtBuffer(buffer, mp3path) {
        return new Promise(resolve => {
            const albumCoverSavePath = mp3path.slice(0,-3) + 'jpg';
            this.ensureDir(albumCoverSavePath);
            fs.writeFile(albumCoverSavePath, buffer, err => {
    ...
    saveSyncedLyrics(lyrics_json, mp3path) {
        let lyricsText = '';

        for (let i = 0; i < lyrics_json.length; i++) {
            if (lyrics_json[i].lrc_timestamp) {
                lyricsText += lyrics_json[i].lrc_timestamp + lyrics_json[i].line + '\r\n';
            } else if (i + 1 < lyrics_json.length) {
                lyricsText += lyrics_json[i + 1].lrc_timestamp + lyrics_json[i].line + '\r\n';
            }
        }
        
        if (lyricsText.trim() !== '') {
            const lyricsFile = mp3path.slice(0, -3) + 'lrc';

            this.ensureDir(lyricsFile);
            fs.writeFileSync(lyricsFile, lyricsText);
        }
    }
