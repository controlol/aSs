const router = require('express').Router(),
      ...
      { getSpotifyToken, getPlaylistTrackIDs, getPlaylistTitle } = require('../functions/spotify'),
      ...
      { concurrentDownloads, optimizedFS, downloadLocation, quality, allowUploads } = require('../../src/config.json')
...
//smloadr settings
const rootFolder = smloadr.setDownloadPath(downloadLocation)
...
router.route('/track').get((req, res) => {
  ...
  return getArl(name)
  .then(arl => {
    return handleDownload(deezerID, playlistID, arl)
...
router.route('/playlist').get((req, res) => {
  ...
  .then(resPlaylist => {
    ...
    let archiveExists = fs.access(archivePath, err => err ? false : true)

    if (!allowUploads) return res.json({success: "Uploading to system not allowed"})
    if (resPlaylist.lastZip > resPlaylist.lastDownload && archiveExists) { //fs.existsSync(archivePath)
    ...
    /*let tasks = resPlaylist.tracks.map(spotifyID => {
      return new Promise(resolve => {
        ...
        .catch(err => reject(new mongoError("no result", "SongMatch", err.errors || err, { spotifyID })))
      })
    })

    return Promise.all(tasks)*/
    .then(() => {
      return createArchive(playlistTitle, tracks, archivePath)
      .then(archivePath => {
        resPlaylist.lastZip = new Date()

        return resPlaylist.save()
        .then(() => res.download(archivePath))
        .catch(err => {
          console.error(err)
...
router.route('/update-playlist-settings').post((req, res) => {
  ...
  if (
    ...
    typeof removedTracks === "undefined"
  ...
  .then(result => {
    ...
    return result.save()
    .then(() => res.json({success: "updated playlist settings"}))
...
let syncPlaylist = () => {
  ...
  return Playlist.find({sync: true}) // find all playlists that need to be synced
  .then(result => {
    ...
    .then(results => {
      results.forEach(result => { if (result.isRejected()) console.log(result.reason()) })

      currentlySyncingPlaylists = false
      console.log("Finished syncing all playlists!")

      return SongMatch.find({ deezerID: "badmatch" })
      .then(result => {
