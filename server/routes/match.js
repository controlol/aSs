...
const router = require('express').Router(),
      ...
      { matchByISRC, matchByQuery, searchTracks, updateMatch, storePlaylist } = require('../functions/match'),
...
router.route('/getmatch').get((req, res) => {
  ...
  .then(result => {
    ...
    Promise.all(tasks)
    .then(() => res.json({match, byISRC, lastDownload}))
  })
  .catch(err => {
...
router.route('/storeplaylist').post((req, res) => {
  ...
  return storePlaylist(playlistID, playlistTitle, trackID, name)
  .then(msg => {
